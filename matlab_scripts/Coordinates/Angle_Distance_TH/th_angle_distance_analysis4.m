...
fig_angle = figure;
fig_angle_dis = figure('units','normalized','outerposition',[0 0 1 1]);
fig_dist = figure;
fig_loc = figure;
...
dif_angle = angles_dp_aux - optimal_aoa;
dif_angle = abs(dif_angle(:));
dif_angle(~index_ap_in) = nan;

dif_angle_hf = dif_angle;
dif_angle_hf(~in_hf) = nan;

dif_angle_lf = dif_angle;
dif_angle_lf(~in_lf) = nan;


set(0, 'CurrentFigure', fig_angle),
subplot(2,2,ii)
histogram(dif_angle_hf(:), 100,'Normalization','probability')
hold on
histogram(dif_angle_lf(:), 100,'Normalization','probability')
title("# of arrays = " + string(subarrays(ii)))
legend("Better HF", "Better LF")

%% Distance
estimated_distance_aux = repmat(estimated_distance,1,1,rotations/subarrays(ii));

% neg if mmWave is longer and pos if mmWave is shorter
dif_distance = estimated_distance_aux - optimal_ftm;
dif_distance(~index_ap_in) = nan;

dif_distance = dif_distance(:);


dif_distance_hf = dif_distance;
dif_distance_hf(~in_hf) = nan;

dif_distance_lf = dif_distance;
dif_distance_lf(~in_lf) = nan;

set(0, 'CurrentFigure', fig_dist),
subplot(2,2,ii)
histogram(dif_distance_hf(:), 100,'Normalization','probability')
hold on
histogram(dif_distance_lf(:), 100,'Normalization','probability')
title("# of arrays = " + string(subarrays(ii)))
legend("Better HF", "Better LF")
% error_all = [];
...
%% In terms of the disagreement

index_dis = disagreement > 2 & disagreement <= 4.5;

dif_angle_hf(~index_dis) = nan;
dif_angle_lf(~index_dis) = nan;

set(0, 'CurrentFigure', fig_angle_dis),
subplot(2,2,ii)
% subplot(2,1,1)
histogram(dif_angle_hf(:), 100,'Normalization','probability')
hold on
...
set(0, 'CurrentFigure', fig_angle)
sgtitle("Angle difference")
print(fig_angle,"plots/plots_difference/angle", "-dpng")
savefig(fig_angle,"plots/plots_difference/angle")

set(0, 'CurrentFigure', fig_angle_dis)
sgtitle("Angle difference")
% set(gca, 'Position', [0 0 1 1]); 
print(fig_angle_dis,"plots/plots_difference/angle_distance_2_4", "-dpng")
savefig(fig_angle,"plots/plots_difference/angle_distance_2_4")

set(0, 'CurrentFigure', fig_dist)
sgtitle("Distance difference")
print(fig_dist,"plots/plots_difference/distance", "-dpng")
savefig(fig_dist,"plots/plots_difference/distance")

set(0, 'CurrentFigure', fig_loc)
sgtitle("Location difference")
print(fig_loc,"plots/plots_difference/location", "-dpng")
savefig(fig_loc,"plots/plots_difference/location")
