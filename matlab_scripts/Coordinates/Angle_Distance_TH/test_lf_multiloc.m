...
mkdir("plots")
mkdir("plots/final_plots_subplot_new_lf")
...
mkdir("mat_files/")
mkdir("mat_files/CDF_data_new_lf")
...
% for scenario = 1
mkdir(strcat("mat_files/CDF_data_new_lf/", scenarios_save(scenario)))
    ...
    for id_router = 1:routers
        for id_rotation = 1:(rotations/subarrays(ii))
            ...
            if (disagreement(id_point, id_router, id_rotation) > th_disagreement)
                estimated_point_x(id_point, id_router, id_rotation) = estimated_point_x_lf_distance_2(id_point);
                estimated_point_y(id_point, id_router, id_rotation) = estimated_point_y_lf_distance_2(id_point);
            else
...
%% Saving the data
save(strcat("mat_files/CDF_data_new_lf/",scenarios_save(scenario),"/all_",num2str(subarrays(ii)),".mat"), "error_multiloc_lf", "error_multiloc_hf", "error_multiloc", "error_mb_bs_mean", "error_mb_bs_ap", "error_hf_bs_mean", "error_hf_bs_ap");
savefig(fig,strcat("plots/final_plots_subplot_new_lf/",scenarios_save(scenario)))
save_PDF_fig(fig,strcat("plots/final_plots_subplot_new_lf/",scenarios_save(scenario)))
