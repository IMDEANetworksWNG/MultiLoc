...
errors_per_deviation_elevation = nan(size(indoor_coordinates.points_x, 1), size(indoor_rx.RX_x, 2), size(rotations, 2));
...
for ii=1:size(indoor_coordinates.points_x, 1)
    ...
    for jj=1:size(indoor_rx.RX_x, 2)
        ...
        for rotation_index=1:size(rotations, 2)
            ...
            elevation = csi_indoor.calculated_aoa(ii, jj, rotations==current_rotation);
            elevation = elevation(1,1);
            errors_per_deviation_elevation(ii, jj, save_index) = elevation;
        end
    end
end
...
legend([rotations_legend], 'Location', "SouthEast")
title("Elevation. NLOS")
ylabel("ECDF")
xlabel("Angle error [deg]")
savefig(fig, "plots/final_plots/rotation_comparison/Elevation_NLOS_Indoor")
save_PDF_fig(fig, "plots/final_plots/rotation_comparison/Elevation_NLOS_Indoor")
