...
for scenario = 1:length(scenarios)
    load(strcat("mat_files/metrics/", scenarios_save(scenario), "/error_multiloc_all"))
    rmse = zeros(size(error_multiloc_all));

    for metric = 1:length(metrics)
        for rot = 1:length(subarrays)

            error_multiloc = error_multiloc_all{metric,rot};
            rmse(metric,rot) = sqrt(mean(error_multiloc.^2));
        end
    end
    figure, bar(metrics,rmse)
    legend(string(subarrays))
    title(scenarios_save(scenario))
    xticklabels(["Distance-LF", "Power ratio", "Mean excess delay"]);
    ylabel("RMSE")
end