...
mkdir("mat_files/")
mkdir("mat_files/metric_alpha")
        
th_disagreement_all = [5,4.5,2,2];


for scenario = 1:length(scenarios)
% for scenario = 2
mkdir(strcat("mat_files/metric_alpha/", scenarios_save(scenario)))
...
%     alpha = 1;
    ...
    error_multiloc_all{find(alpha == alphas),ii} = error_multiloc(:);
...
%     legend("MultiLoc LF","MultiLoc HF", "MultiLoc")
    ...
    %     xticks(1:16)
    % end
%     sgtitle(scenarios_save(scenario) + ". alpha " + string(alpha))
    %% Saving the data
...
save(strcat("mat_files/metric_alpha/",scenarios_save(scenario),"_error_multiloc_all"), "error_multiloc_all", "alphas");
end
