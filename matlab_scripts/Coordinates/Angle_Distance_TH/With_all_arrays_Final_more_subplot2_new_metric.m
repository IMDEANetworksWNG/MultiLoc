...
for id_point = 1:n_points
    for id_router = 1:routers
        for id_rotation = 1:(rotations/subarrays(ii))
            ...
            % the HF data
            rule = (disagreement(id_point, id_router, id_rotation) > th_disagreement) || dif_angle(id_point, id_router, id_rotation) > angle_diff;
            if (rule)
