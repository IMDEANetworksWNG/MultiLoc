function [all_gains, all_delays, num_estimated_paths,matrix_estimated_gains,matrix_estimated_delay] = matrix_pencil(multipath_channel,P,unit_circle_toll)
...
%         H_n = transpose(multipath_channel(antenna,z_left+1:end-z_right));
        H_n = transpose(multipath_channel);
        % Hankel matrix:
        K = num_subcarriers-P;
        Hankel_matrix = [];
        for k = 1:K
        ...
        all_gains(1:length(gains)) = transpose(gains);
        all_delays(1:length(gains)) = transpose(delays);
        num_estimated_paths = length(gains);
%     end
end
