...
% for scenario = 2
mkdir(strcat("mat_files/metrics/", scenarios_save(scenario)))
...
% load the metrics LF metrics
load(strcat("mat_files/metrics/metrics_",scenarios(scenario),".mat"))
...
elseif (scenario == 3)
    ...
    power_ratio = power_ratio(:, router_indexes);
    mean_excess_delay = mean_excess_delay(:, router_indexes);
...
metric_all = zeros(n_points,routers,3);
metric_all(:,:,1) = estimated_distance;
metric_all(:,:,2) = power_ratio;
metric_all(:,:,3) = mean_excess_delay;
metric_str = ["Distance", "Power ratio", "Mean excess delay"];
...
%     end
    ...
    %     estimated_point_y_aux = squeeze(estimated_point_y(:,:,id_rotation));
        metric = squeeze(metric_all(:,:,id_metric));
        metric(index_nan) = nan;

        min_distance = nanmin(metric,[],2);
        index_in_disagreement = min_distance == metric;
    ...
    %     end
        ...
        % this means no HF converage --> we take LF information
    ...
    error_multiloc_all{id_metric, ii} = error_multiloc(:);
    ...
    sgtitle(metric_str(id_metric) + ". " + scenarios_save(scenario))
    set(fig, 'CreateFcn', 'set(gcbo,''Visible'',''on'')'); 
    savefig(fig,strcat("plots/metric_subplots/",scenarios_save(scenario),metric_str(id_metric)))
    save_PDF_fig(fig,strcat("plots/metric_subplots/",scenarios_save(scenario),metric_str(id_metric)))
    close all
    
end

save(strcat("mat_files/metrics/", scenarios_save(scenario), "/error_multiloc_all"), "error_multiloc_all")
end
