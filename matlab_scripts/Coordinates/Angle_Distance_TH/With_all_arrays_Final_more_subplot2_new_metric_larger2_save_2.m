...
% mkdir("plots")
% mkdir("plots/data_error_mmWave_larger")
...
mkdir("mat_files/")
mkdir("mat_files/data_error_mmWave_larger")
...
error_multiloc_all = {};
% for scenario = 2
mkdir(strcat("mat_files/data_error_mmWave_larger/", scenarios_save(scenario)))
...
for id_point = 1:n_points
    for id_router = 1:routers
        for id_rotation = 1:(rotations/subarrays(ii))
            ...
            rule_aoa = abs(angles_dp(id_point, id_router) - calculated_aoa_rotation(id_point, id_router, id_rotation)) > angle_diff;
...
error_multiloc_all{ii,find(angle_diff == angles_diff)} = error_multiloc(:);
...
save(strcat("mat_files/data_error_mmWave_larger/", scenarios_save(scenario), "/error_multiloc_all"), "error_multiloc_all", "angles_diff")
end
