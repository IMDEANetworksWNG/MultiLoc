...
rotations = 90:45:225;
rotations_str = repmat(string(rotations.'),1,2);
client_rotation = strcat(clients_str,"_pan_",rotations_str);
cli_ids = cellstr((client_rotation(:)).')
...
for point_id=1:size(measurement_points, 2)
    ...
    path = ['raw_data/in_out/aoa_and_ftm/' num2str(measurement_point) '/'];
    ...
    for ap_id=ap_ids
        ...
        for cli_id=1
            ...
            % Read the raw data
            filename = ['cli_' num2str(ap_id) '_ap_' cli_ids{8} '_AOA.txt'];
...
mkdir("mat_files/in_out/HF")
mkdir("mat_files/in_out/HF/CSI")
mkdir("mat_files/in_out/HF/FTM")

out_path = 'mat_files/in_out/HF/';
...
router = 2;
rotation = 8;
calculated_aoa(measurement_point,router,rotation) =  Az_estimated(1);
azimut_raw{measurement_point,router,rotation} = Az_estimated
elevation_raw{measurement_point,router,rotation} = El_estimated
csi_raw{measurement_point,router,rotation} = csi_data
power_raw{measurement_point,router,rotation} = power
magnitudes_raw{measurement_point,router,rotation} = magnitudes
save([out_path 'CSI/csi_in_out2.mat'], 'calculated_aoa', 'csi_raw', 'magnitudes_raw', 'azimut_raw', 'elevation_raw', 'power_raw');
...
% power_raw(:, :, [1:8])      = power_raw(:, :, [I]);
% tof_raw(:, :, [1:8])        = tof_raw(:, :, [I]);
% save([out_path 'CSI/csi_in_out.mat'], 'calculated_aoa', 'csi_raw', 'magnitudes_raw', 'azimut_raw', 'elevation_raw', 'power_raw');
% save([out_path 'FTM/ftm_in_out.mat'], 'calculated_distance', 'tof_raw');
