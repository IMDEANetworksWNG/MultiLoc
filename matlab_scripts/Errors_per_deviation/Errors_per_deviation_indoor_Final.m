...
mkdir plots
mkdir plots/rotation_comparison
...
errors_per_deviation_los_elevation  = nan(size(indoor_coordinates.points_x, 1), size(indoor_rx.RX_x, 2), size(rotations, 2));
errors_per_deviation_nlos_elevation = nan(size(indoor_coordinates.points_x, 1), size(indoor_rx.RX_x, 2), size(rotations, 2));
    ...
    for jj=1:size(indoor_rx.RX_x, 2)
        ...
        for rotation_index=1:size(rotations, 2)
            ...
            if LOS_RX(ii, jj) == 1
                ...
                elevation = csi_indoor.elevation_raw(ii, jj, rotations==current_rotation);
                if (~isempty(elevation{1,1}))
                    elevation = elevation{1,1}(1,1);
                    errors_per_deviation_los_elevation(ii, jj, save_index)  = elevation;
                end
            else
                ...
                if (~isempty(elevation{1,1}))
                    elevation = elevation{1,1}(1,1);
                    errors_per_deviation_nlos_elevation(ii, jj, save_index)  = elevation;
                end
            end
        end
    end
...
hold off
xlabel('(a) Azimuth error [°] LOS','fontweight','bold')
ylabel('ECDF')
title('')
legend([rotations_legend 'LF'], 'Location', "northoutside",'NumColumns',5)
xlim([0 160])
yticks([0:0.2:1])
...
% set(gca,'FontSize',18)
MagInset_Grid_On(fig, -1, [0 10 0 1], [75 150 0.15 0.6], {'NW','NW';'SE','SE'}, [0:5:10], [0:0.5:1]);
% set(gca,'FontSize',18)
% fleg = legend('figure()');
...
savefig(fig,'plots/rotation_comparison/AoA_LOS_Indoor');
...
hold off
xlabel('(d) Azimuth error [°] NLOS','fontweight','bold')
ylabel('')
...
savefig(fig,'plots/rotation_comparison/AoA_NLOS_Indoor');


%% Elevation LOS

errors_per_deviation_los_elevation(:,2,:) = errors_per_deviation_los_elevation(:,2,:) - 6;
errors_per_deviation_nlos_elevation(:,2,:) = errors_per_deviation_nlos_elevation(:,2,:) - 6;

errors_per_deviation_los_elevation(:,3,:) = errors_per_deviation_los_elevation(:,3,:) - 3;
errors_per_deviation_nlos_elevation(:,3,:) = errors_per_deviation_nlos_elevation(:,3,:) - 3;

errors_per_deviation_los_elevation(:,4,:) = errors_per_deviation_los_elevation(:,4,:) + 3.5;
errors_per_deviation_nlos_elevation(:,4,:) = errors_per_deviation_nlos_elevation(:,4,:) + 3.5;
...
for rotation_index=rotations_index
    ...
    aux = abs(errors_per_deviation_los_elevation(:, :, rotation_index));
    h = cdfplot(aux(:));
...
% legend([rotations_legend], 'Location', "SouthEast")
% title("Elevation. LOS")
% ylabel("ECDF")
xlabel("Elevation error [º] LOS",'fontweight','bold')

xlim([0 48])
xticks(0:15:45)
yticks([0:0.2:1])

xlabel('(b) Elevation error [°] LOS','fontweight','bold')
ylabel('')
...
MagInset_Grid_On(fig, -1, [0 10 0 1], [22.5 45 0.15 0.6], {'NW','NW';'SE','SE'}, [0:5:10], [0:0.5:1]);

fleg = legend('figure()');
set(fleg,'visible','off')


% matlab2tikz('plots/final_plots/rotation_comparison/Elevation_LOS_Indoor.tikz');
savefig(fig,'plots/rotation_comparison/Elevation_LOS_Indoor');

% savefig(fig, "plots/final_plots/rotation_comparison/Elevation_LOS_Indoor")
% save_PDF_fig(fig, "plots/final_plots/rotation_comparison/Elevation_LOS_Indoor")
...
% 
% end
% legend(["AP 1", "AP 2", "AP 5"], 'Location', "SouthEast")
% title("Elevation. LOS Oracle")
...
for rotation_index=rotations_index
    ...
    aux = abs(errors_per_deviation_nlos_elevation(:, :, rotation_index));
    h = cdfplot(aux(:));
...
xlabel('(e) Elevation error [°] NLOS','fontweight','bold')
ylabel('')
title('')
...
% matlab2tikz('plots/final_plots/rotation_comparison/Elevation_NLOS_Indoor.tikz');
savefig(fig,'plots/rotation_comparison/Elevation_NLOS_Indoor');
...
MagInset_Grid_On(fig, -1, [0 1 0 1], [15 30 0.15 0.6], {'NW','NW';'SE','SE'}, [0:0.5:1], [0:0.5:1]);

fleg = legend('figure()');
set(fleg,'visible','off')

% matlab2tikz('plots/final_plots/rotation_comparison/ToF_LOS_Indoor.tikz');
savefig(fig,'plots/rotation_comparison/ToF_LOS_Indoor');
...
hold off
xlabel('(f) ToF error [m] NLOS','fontweight','bold')
ylabel('')
...
savefig(fig,'plots/rotation_comparison/ToF_NLOS_Indoor');
