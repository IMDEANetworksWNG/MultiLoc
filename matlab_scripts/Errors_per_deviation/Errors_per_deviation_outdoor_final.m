...
% Calibration
calibration = load('mat_files/outdoor/HF/CSI/calibration2.mat');
...
errors_per_deviation_elevation = nan(size(outoor_coordinates.points_x, 2), size(outdoor_rx.RX_x, 2), size(rotations, 2));
...
%             errors_per_deviation(ii, jj, save_index) = abs(csi_outdoor.calculated_aoa(ii, jj, rotations==current_rotation) - -1*true_angles(ii, jj, rotations==current_rotation));
            elevation = csi_outdoor.elevation_raw(ii, jj, rotations==current_rotation);
...
errors_per_deviation_elevation = errors_per_deviation_elevation + cal;
...
for rotation_index=1:size(rotations, 2)
    ...
    aux = abs(errors_per_deviation_elevation(:, :, rotation_index));
    h = cdfplot(aux(:));
...
hold off
xlabel('Error in degrees')
title('[Outdoor] Elevation errors if we chose the wrong rotation by N degrees')
legend(string(rotations), 'Location', "SouthEast")
matlab2tikz('plots/final_plots/rotation_comparison/El_LOS_Outdoor.tikz');
