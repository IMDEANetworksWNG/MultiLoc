...
mkdir("plots")
mkdir("plots/subplot_difference_axis")
...
% for scenario = 2
mkdir(strcat("mat_files/CDF_data/", scenarios_save(scenario)))
...
error_multiloc_lf_x = points_x - estimated_point_x_lf_distance_2;
error_multiloc_lf_y = points_y - estimated_point_y_lf_distance_2;
...
error_multiloc_hf_x = points_x - estimated_point_x_hf_tof;
error_multiloc_hf_y = points_y - estimated_point_y_hf_tof;
...
error_multiloc_x = points_x - estimated_point_x_hf_lf_2;
error_multiloc_y = points_y - estimated_point_y_hf_lf_2;

% figure
plot(error_multiloc_lf_x, error_multiloc_lf_y, "b*")
hold on
plot(error_multiloc_hf_x, error_multiloc_hf_y, "r*")
title(strcat("# of subarrays ",string(subarrays(ii)))); 
legend("MultiLoc LF","MultiLoc HF")
sgtitle(scenarios_save(scenario))

%% Saving the data
% save(strcat("mat_files/CDF_data/",scenarios_save(scenario),"/all_",num2str(subarrays(ii)),".mat"), "error_multiloc_lf", "error_multiloc_hf", "error_multiloc", "error_mb_bs_mean", "error_mb_bs_ap", "error_hf_bs_mean", "error_hf_bs_ap");
savefig(fig,strcat("plots/subplot_difference_axis/",scenarios_save(scenario)))
save_PDF_fig(fig,strcat("plots/subplot_difference_axis/",scenarios_save(scenario)))
end
end
