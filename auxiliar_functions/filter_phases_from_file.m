...
function [offset_phase, csi_filtered] = filter_phases_from_file(file, antenna)
    [~, phases, ~] = Parse_raw_data(file);
    ...
    csi_filtered = csi_data_test .* S_phase(index_max);
    %figure, plot(angle(csi_data_test_aux));

    % apply guaxian mixture model
    GM = fitgmdist(angle(csi_filtered),2);
    ...
    if (mu_sorted(2) < mu_sorted(1))
        index_out = angle(csi_filtered) < phase_middle;
    else
        index_out = angle(csi_filtered) > phase_middle;
    end
    ...
    csi_filtered(index_out) = csi_filtered(index_out) .* conj(exp(1i*grid_pi(index_min_mu)));

    % csi_data_test_aux = csi_data_test .* exp(-1i*offset_mu);
    %figure, plot(angle(csi_data_test_aux));

    % it should be center at 0 or closer to 0
    mean(angle(csi_filtered))
