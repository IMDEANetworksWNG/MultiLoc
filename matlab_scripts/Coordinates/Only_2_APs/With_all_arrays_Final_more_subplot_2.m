...
load("mat_files/Coordinates/data_coordinates_multiband_baseline.mat")

% load the hf bbaseline
load("mat_files/Coordinates/data_coordinates_hf_bs.mat")
...
% save(['matlab_scripts/Coordinates/Only_2_APs/lf_distance_2.mat'], "estimated_point_x_lf_distance_2", "estimated_point_y_lf_distance_2");
...
error_hf_hf_lf_new = sqrt((points_x - estimated_point_x_hf_lf_2).^2 + (points_y - estimated_point_y_hf_lf_2).^2);
...
index_nan_error = isnan(error_mb_bs_mean);
error_mb_bs_mean(index_nan_error_tof) = Inf;
...
estimated_point_x_mb_bs_ap = nan(n_points, rotations/subarrays(ii));
estimated_point_y_mb_bs_ap = nan(n_points, rotations/subarrays(ii));
for id_rotation = 1:(rotations/subarrays(ii))
...
index_nan_error = isnan(error_mb_bs_ap);
error_mb_bs_ap(index_nan_error) = Inf;
...
index_nan_error = isnan(error_hf_bs_mean);
error_hf_bs_mean(index_nan_error) = Inf;
...
index_nan_error = isnan(error_hf_bs_ap);
error_hf_bs_ap(index_nan_error_tof) = Inf;
...
legend("MultiLoc LF","MultiLoc HF", "MultiLoc", "MB-BS mean", "MB-BS ap", "HF-BS mean", "HF-BS ap", "Location", "southeast")
...
save(['matlab_scripts/Coordinates/Only_2_APs/all_indoor_' num2str(subarrays(ii)) '.mat'], "error_lf_distance", "error_hf_hf_lf_new", "error_hf_tof");
