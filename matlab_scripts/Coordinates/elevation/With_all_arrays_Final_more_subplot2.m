...
for scenario = 4
% for scenario = 2
mkdir(strcat("mat_files/CDF_data_elevation/", scenarios_save(scenario)))
...
% load the HF data
load(strcat("mat_files/Coordinates/data_coordinates_hf_",scenarios(scenario),"_elevation.mat"))
% load the LF data
...
elseif (scenario == 3)
    ...
    estimated_point_z_hf = estimated_point_z_hf(:, router_indexes, :);
...
estimated_point_z_hf_rotations = nan(n_points,routers,rotations/subarrays(ii));

estimated_point_z_lf = zeros(size(estimated_point_x_lf));
...
for id_point = 1:n_points
    for id_router = 1:routers
        for id_rotation = 1:(rotations/subarrays(ii))
            ...
            estimated_point_z_hf_aux = estimated_point_z_hf(id_point, id_router, id_rotation + index_subarrays{ii});
            ...
            estimated_point_z_lf_aux = estimated_point_z_lf(id_point, id_router);

            % take the one that better agree with LF
            distance_arrays = sqrt((estimated_point_x_hf_aux - estimated_point_x_lf_aux).^2 + (estimated_point_y_hf_aux - estimated_point_y_lf_aux).^2 + (estimated_point_z_hf_aux - estimated_point_z_lf_aux).^2);
...
%                 [~, index_min_rotation] = min(distance_arrays);
                ...
                estimated_point_z_hf_aux = estimated_point_z_hf_aux(index_min_rotation);

                distance_lf_hf = sqrt((estimated_point_x_hf_aux - estimated_point_x_lf_aux)^2 + (estimated_point_y_hf_aux - estimated_point_y_lf_aux)^2 + (estimated_point_z_hf_aux - estimated_point_z_lf_aux)^2);
                ...
                estimated_point_z_hf_rotations(id_point, id_router, id_rotation) = estimated_point_z_hf_aux;
...
estimated_point_z = zeros(n_points,routers,rotations/subarrays(ii));
    ...
    for id_router = 1:routers
        for id_rotation = 1:(rotations/subarrays(ii))
            ...
            if (disagreement(id_point, id_router, id_rotation) > th_disagreement)
                ...
                estimated_point_z(id_point, id_router, id_rotation) = estimated_point_z_lf(id_point, id_router);
                ...
                estimated_point_z(id_point, id_router, id_rotation) = estimated_point_z_hf_rotations(id_point, id_router, id_rotation);
            ...
            if (disagreement(id_point, id_router, id_rotation)  == Inf)
                ...
                estimated_point_z(id_point, id_router, id_rotation) = nan;
...
estimated_point_z_hf_tof = nan(n_points, rotations/subarrays(ii));
...
for id_point = 1:n_points
    ...
    for id_rotation = 1:(rotations/subarrays(ii))
        ...
        if(~isnan(candidate_ap) && ~isnan(candidate_subarray))
            ...
            estimated_point_z_hf_tof(id_point, id_rotation) = estimated_point_z_hf(id_point, candidate_ap, candidate_subarray);
            chosen_ap_with_hf(id_point, id_rotation) = candidate_ap;
        end
    end
end

error_multiloc_hf = sqrt((points_x - estimated_point_x_hf_tof).^2 + (points_y - estimated_point_y_hf_tof).^2 + (points_z - estimated_point_z_hf_tof).^2);
...
estimated_point_z_hf_lf_2 = zeros(n_points, rotations/subarrays(ii));
...
for id_rotation = 1:(rotations/subarrays(ii))
    ...
    estimated_point_z_aux = squeeze(estimated_point_z(:,:,id_rotation));
%     estimated_point_x_aux = squeeze(estimated_point_x(:,:,id_rotation));
...
%     end
    ...
    estimated_point_z_hf_lf_2(:,id_rotation) = nansum(estimated_point_z_aux.*index_in_disagreement,2);
    ...
    if (sum(index_0) > 0)
        ...
        estimated_point_z_hf_lf_2(index_0, id_rotation) = 0;
        ...
        estimated_point_z(index_0,1,id_rotation) = 0;
...
% estimated_point_z_hf_lf_2 = zeros(size(estimated_point_z_hf_lf_2));
error_multiloc = sqrt((points_x - estimated_point_x_hf_lf_2).^2 + (points_y - estimated_point_y_hf_lf_2).^2 + (points_z - estimated_point_z_hf_lf_2).^2);
...
%% Saving the data
save(strcat("mat_files/CDF_data_elevation/",scenarios_save(scenario),"/all_",num2str(subarrays(ii)),".mat"), "error_multiloc_lf", "error_multiloc_hf", "error_multiloc");
savefig(fig,strcat("plots/final_plots_subplot_elevation/",scenarios_save(scenario)))
save_PDF_fig(fig,strcat("plots/final_plots_subplot_elevation/",scenarios_save(scenario)))
if(id_rotation == 1)
    save(strcat("mat_files/CDF_data_elevation/",scenarios_save(scenario),"/chosen_rotation_with_hf"), "chosen_rotation_with_hf", "index_ap_in");
end
